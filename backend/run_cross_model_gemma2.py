#!/usr/bin/env python3
"""PromptGuard Ai — Final Gemma 2 9B (2024) Cross-Model Benchmark Runner.

Executes the 72 comparable cases (36 adversarial, 36 benign) on local gemma2:9b
...
GEMMA2_DIR = BACKEND_DIR / "benchmark_results" / "cross_model" / "gemma2_9b"
GEMMA2_DB_PATH = GEMMA2_DIR / "gemma2_9b.db"
GEMMA2_LEDGER_PATH = GEMMA2_DIR / "ledger.json"

PREFLIGHT_DIR = BACKEND_DIR / "benchmark_results" / "cross_model" / "gemma2_9b_preflight"
...
def load_ledger(ledger_path: Path) -> dict[str, Any]:
    if ledger_path.exists():
        with open(ledger_path, "r", encoding="utf-8") as f:
            return json.load(f)
    return {
        "benchmark": "PromptGuard Ai — Cross-Model Benchmark (Gemma 2 9B)",
        "model": MODEL_TAG,
...
async def run_preflight():
    ...
    results = {}
    try:
        for cid in preflight_cases:
            print(f"\n--- [Preflight] Processing Case {cid} ---")
            ...
            if fam_enum is not None:
                attack_prompt = frozen_ledger[cid]["generated_attack_prompt"]
            else:
            ...
            row = db.query(ExperimentRun).filter_by(id=resp.experiment_id).one()
            
            print(f"[{cid}] Baseline status: {row.baseline_status}, Defended status: {row.defended_status}")
            print(f"[{cid}] Baseline latency: {row.baseline_latency_ms:.1f}ms, Defended latency: {row.defended_latency_ms:.1f}ms")
            assert row.baseline_status == "completed" and row.defended_status == "completed", f"Preflight case {cid} failed model execution"

            # Run evaluation
            print(f"[{cid}] Running evaluator...")
            eval_result = evaluate_experiment_run(row)
            eval_dict = result_to_dict(eval_result)
            row.evaluation_json = json.dumps(eval_dict)
            db.commit()

            print(f"[{cid}] Evaluator method: {eval_result.evaluator_method}")
            print(f"[{cid}] Evaluator rationale: {eval_result.evaluator_rationale[:80]}...")
            ...
            save_ledger(PREFLIGHT_LEDGER_PATH, ledger)
...
async def run_benchmark(limit: Optional[int] = None, resume: bool = True):
    ...
    ledger = load_ledger(GEMMA2_LEDGER_PATH)
    db = get_db_session(GEMMA2_DB_PATH)
        ...
        for idx, case in enumerate(comparable_cases, 1):
            ...
            if (
                ...
                if not ledger_entry or not ledger_entry.get("evaluation_completed"):
                    ...
                    save_ledger(GEMMA2_LEDGER_PATH, ledger)
                continue
            ...
            # State B: Model outputs completed, but evaluation missing or failed
            if (
                existing_row
                and existing_row.baseline_status == "completed"
                and existing_row.defended_status == "completed"
                ...
                # Clean up partial/failed row if any
                if existing_row:
...
def print_status():
    ...
    print("=== Gemma 2 9B Cross-Model Benchmark Status ===")
...
def analyze_results() -> dict[str, Any]:
    ...
    analysis_path = GEMMA2_DIR / "analysis_summary.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PromptGuard Ai — Gemma 2 9B Cross-Model Benchmark Runner")
